...
public class QueUpdateGrowthThread extends Thread {
    ...
    QueUpdateGrowthThread(UpdateGrowthObject rankingOjbect) {
    ...
    public void run() {
        ...
        while(true) {
            synchronized (this) {
                if (id <= rankingOjbect.getId()) {
                    rankingOjbect.updateId();
                    id = rankingOjbect.getId();
                }
            }
            if(id >= rownum) {
            ...
            original_data = mysql.selectDatalabPlusTotal3(conn, st, rs, "select * from datalab_plus where id=" + id);;
